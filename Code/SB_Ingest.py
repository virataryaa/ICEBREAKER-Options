"""
SB_Ingest.py -- SB Sugar #11 Options (ICE Connect)
Fetches last 10 days, upserts into parquet.
Strike range: ATM +/- 20 strikes (25-point increments) centred on %SB 1! last settle.
%SB 1! returns cts/lb; symbol strikes are in 1/100 cts/lb (e.g. 17.50 -> 1750).
...
OUT_PATH    = Path(__file__).resolve().parent.parent / "Database" / "SB_options_ice.parquet"
...
N_MONTHS    = 12
ATM_WING    = 20
STRIKE_STEP = 25   # symbol-unit increments (1/100 cts/lb); 25 = 0.25 cts/lb
...
def get_atm_strike() -> int:
    """Fetch %SB 1! last settle (cts/lb), convert to symbol units, round to STRIKE_STEP."""
    raw = ice.get_timeseries(
        ["%SB 1!"], ["Settle"], granularity="D",
        start_date=(datetime.date.today() - datetime.timedelta(days=5)).isoformat(),
        end_date=TODAY
    )
    if not raw or len(raw) < 2:
        raise RuntimeError("Could not fetch %SB 1! price for ATM calculation")
    last_settle_cts = next(
        (float(row[1]) for row in reversed(raw[1:]) if row[1] is not None),
        None
    )
    if last_settle_cts is None:
        raise RuntimeError("No valid settle found for %SB 1!")
    # %SB 1! is in cts/lb; symbol strikes are in 1/100 cts/lb
    last_settle_sym = last_settle_cts * 100
    atm = round(last_settle_sym / STRIKE_STEP) * STRIKE_STEP
    print(f"SB 1! last settle: {last_settle_cts:.2f} cts/lb  ->  ATM strike: {atm} ({atm/100:.2f} cts/lb)")
    ...
    if ATM_PATH.exists():
        try:
            ...
            pass
    existing["SB"]      = round(atm / 100, 4)  # store in cts/lb; matches parquet strikes
    existing["updated"] = TODAY
    ATM_PATH.write_text(json.dumps(existing))
    return atm  # integer in symbol units
...
def make_symbols(month_code: str, year: int, strikes: list[int]) -> list[str]:
    ...
    syms = []
    for s in strikes:
        syms.append(f"SB {month_code}{yy}C{s}")
        syms.append(f"SB {month_code}{yy}P{s}")
    return syms
...
def parse_symbol(sym: str) -> dict | None:
    try:
        ...
        return {
            ...
            "strike":       strike_sym / 100,   # store in cts/lb
...
def build() -> pd.DataFrame:
    ...
    print(f"Contracts: {len(contracts)} | Strikes: ATM {atm} +/-{ATM_WING} (step {STRIKE_STEP}) | Batches: {len(all_batches)}")
